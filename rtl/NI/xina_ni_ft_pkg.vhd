...
package xina_ni_ft_pkg is
  ...
  ------------------------------------------------------------------------------
  -- Self-Test FT Configuration
  ------------------------------------------------------------------------------
  -- This section groups the FT knobs used by the TG/TM/LB self-test path and
  -- the closed-box observability blocks.
  --
  -- Naming convention:
  -- * *_INJECT_ERROR   : compile-time permission to inject faults into the block
  --
  -- Runtime note:
  -- * For TG/TM/LB/NI/OBS blocks, correction behavior follows the runtime
  --   OBS_*_CORRECT_ERROR fanout generated by the UART D/E command.
  -- * The two UART shell FT blocks are the exception: their correction is
  --   forced on in RTL so the control/reporting path cannot disable itself.
  ...
  ------------------------------------------------------------------------------
  -- Shared / Closed-Box Observability Blocks
  ------------------------------------------------------------------------------
  -- Hamming FIFO control-state TMR (stage_valid + fifo_count in buffer_fifo_ham).
  constant c_ENABLE_HAM_FIFO_CTRL_TMR : boolean := true;

  -- Start/done observation sequencer. Correction still follows the runtime D/E fanout.
  constant c_ENABLE_OBS_START_DONE_CTRL_TMR : boolean := true;

  -- UART shell FT blocks. Their correction is forced on in RTL, so there is no
  -- package-level correction switch for them anymore.

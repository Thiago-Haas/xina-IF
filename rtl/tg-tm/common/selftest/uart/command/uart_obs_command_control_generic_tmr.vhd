...
-- TMR wrapper for uart_obs_command_control_generic.
entity uart_obs_command_control_generic_tmr is
  port (
    ACLK    : in  std_logic;
...
architecture rtl of uart_obs_command_control_generic_tmr is
  type tmr_sl_t is array (2 downto 0) of std_logic;

  signal run_enable_w        : tmr_sl_t;
  signal reset_pulse_w       : tmr_sl_t;
  signal correction_enable_w : tmr_sl_t;
  signal voter_a_w           : std_logic_vector(2 downto 0);
  signal voter_b_w           : std_logic_vector(2 downto 0);
  signal voter_c_w           : std_logic_vector(2 downto 0);
  signal corrected_w         : std_logic_vector(2 downto 0);
  signal error_bits_w        : std_logic_vector(2 downto 0);
...
begin
  ...
  begin
    u_control: entity work.uart_obs_command_control_generic
      port map(
        ...
        correction_enable_o => correction_enable_w(i)
      );
  end generate;

  voter_a_w <= correction_enable_w(0) & reset_pulse_w(0) & run_enable_w(0);
  voter_b_w <= correction_enable_w(1) & reset_pulse_w(1) & run_enable_w(1);
  voter_c_w <= correction_enable_w(2) & reset_pulse_w(2) & run_enable_w(2);
  ...
  correction_enable_o <= corrected_w(2);
end architecture;

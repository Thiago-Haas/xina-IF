...
-- Generic UART report controller for OBS/self-test blocks.
entity uart_obs_encode_ctrl_generic is
  generic(
...
architecture rtl of uart_obs_encode_ctrl_generic is
  ...
  signal char_index_r : unsigned(G_CHAR_INDEX_WIDTH - 1 downto 0) := (others => '0');
        ...
        case state_r is
          ...
          when C_ST_WAIT_DONE =>
            if uart_tdone_i = '1' then
              if char_index_r = to_unsigned(G_LINE_LEN - 1, char_index_r'length) then
                state_r <= C_ST_IDLE;
              else

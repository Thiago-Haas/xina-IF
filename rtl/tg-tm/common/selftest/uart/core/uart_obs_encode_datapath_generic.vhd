...
-- Generic UART report datapath.
entity uart_obs_encode_datapath_generic is
  generic(
  ...
  port(
    ...
    tm_count_single_err_o : out std_logic := '0';
    tm_count_double_err_o : out std_logic := '0';
    flags_seen_single_err_o : out std_logic := '0';
    flags_seen_double_err_o : out std_logic := '0';
    event_flags_single_err_o : out std_logic := '0';
    event_flags_double_err_o : out std_logic := '0';
    report_flags_single_err_o : out std_logic := '0';
    report_flags_double_err_o : out std_logic := '0';
    tx_data_o  : out std_logic_vector(7 downto 0)
  );
end entity;

architecture rtl of uart_obs_encode_datapath_generic is
  ...
  signal tm_count_r : std_logic_vector(G_TM_COUNT_WIDTH - 1 downto 0);
  signal event_flags_seen_r : std_logic_vector(G_FLAGS_WIDTH - 1 downto 0);
  signal event_flags_r : std_logic_vector(G_FLAGS_WIDTH - 1 downto 0);
  signal flags_r    : std_logic_vector(G_FLAGS_WIDTH - 1 downto 0);
  signal new_event_w : std_logic;
  signal tm_count_single_err_w : std_logic;
  signal tm_count_double_err_w : std_logic;
  signal tm_count_enc_w : std_logic_vector(G_TM_COUNT_WIDTH + get_ecc_size(G_TM_COUNT_WIDTH, p_USE_UART_HAMMING_DOUBLE_DETECT) - 1 downto 0);
  signal event_flags_seen_single_err_w : std_logic;
  signal event_flags_seen_double_err_w : std_logic;
  signal event_flags_seen_enc_w : std_logic_vector(G_FLAGS_WIDTH + get_ecc_size(G_FLAGS_WIDTH, p_USE_UART_HAMMING_DOUBLE_DETECT) - 1 downto 0);
  signal event_flags_single_err_w : std_logic;
  signal event_flags_double_err_w : std_logic;
  signal event_flags_enc_w : std_logic_vector(G_FLAGS_WIDTH + get_ecc_size(G_FLAGS_WIDTH, p_USE_UART_HAMMING_DOUBLE_DETECT) - 1 downto 0);
  signal report_flags_single_err_w : std_logic;
  signal report_flags_double_err_w : std_logic;
  signal report_flags_enc_w : std_logic_vector(G_FLAGS_WIDTH + get_ecc_size(G_FLAGS_WIDTH, p_USE_UART_HAMMING_DOUBLE_DETECT) - 1 downto 0);
  signal report_flags_data_w : std_logic_vector(G_FLAGS_WIDTH - 1 downto 0);
...
begin
  ...
  u_event_flags_seen_hamming_register: entity work.hamming_register
    generic map(
      DATA_WIDTH     => G_FLAGS_WIDTH,
      HAMMING_ENABLE => p_USE_UART_FLAGS_SEEN_HAMMING,
      DETECT_DOUBLE  => p_USE_UART_HAMMING_DOUBLE_DETECT,
      RESET_VALUE    => (G_FLAGS_WIDTH - 1 downto 0 => '0'),
      INJECT_ERROR   => p_USE_UART_FLAGS_SEEN_HAMMING_INJECT_ERROR
    )
    port map(
      correct_en_i => flags_seen_correct_error_i,
  ...
  u_event_flags_hamming_register: entity work.hamming_register
    ...
    port map(
      correct_en_i => event_flags_correct_error_i,
  ...
  u_report_flags_hamming_register: entity work.hamming_register
    ...
    port map(
      correct_en_i => report_flags_correct_error_i,
      write_en_i   => load_report_i,
  ...
  tm_count_single_err_o <= tm_count_single_err_w;
  tm_count_double_err_o <= tm_count_double_err_w;
  flags_seen_single_err_o <= event_flags_seen_single_err_w;
  flags_seen_double_err_o <= event_flags_seen_double_err_w;
  event_flags_single_err_o <= event_flags_single_err_w;
  event_flags_double_err_o <= event_flags_double_err_w;
  report_flags_single_err_o <= report_flags_single_err_w;
  report_flags_double_err_o <= report_flags_double_err_w;
  ...
  begin
    ...
    if idx_v < G_PREFIX'length then
      tx_data_o <= f_char(G_PREFIX(idx_v + 1));
    elsif idx_v < G_PREFIX'length + C_TM_HEX_DIGITS then
      tx_data_o <= f_hex(f_nibble(tm_count_r, idx_v - G_PREFIX'length));
    elsif idx_v < G_PREFIX'length + C_TM_HEX_DIGITS + G_MID'length then
      tx_data_o <= f_char(G_MID(idx_v - G_PREFIX'length - C_TM_HEX_DIGITS + 1));
    elsif idx_v < G_PREFIX'length + C_TM_HEX_DIGITS + G_MID'length + C_FLAGS_HEX_DIGITS then
      tx_data_o <= f_hex(f_nibble(flags_r, idx_v - G_PREFIX'length - C_TM_HEX_DIGITS - G_MID'length));
    end if;
  end process;
end architecture;

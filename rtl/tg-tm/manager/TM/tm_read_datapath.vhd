...
--  * Expected payload is generated by a tiny LFSR + small index inside tm_read_lfsr
--  * No 32-bit LFSR state register here
--  * Comparator checks only the changing LSB field ({idx,lfsr}) to save LUTs
...
architecture rtl of tm_read_datapath is
  ...
  constant c_CMP_BITS  : positive := c_LFSR_BITS + p_INDEX_BITS;

  signal w_expected : std_logic_vector(c_AXI_DATA_WIDTH - 1 downto 0);
begin
  -- Make sure compare slice fits.
  assert c_CMP_BITS <= c_AXI_DATA_WIDTH
    report "tm_read_datapath: c_LFSR_BITS + p_INDEX_BITS must be <= c_AXI_DATA_WIDTH"
    severity failure;
  ...
  -- Comparator only checks the changing LSB field to save LUTs.
  -- Trade-off: bit-flips in the upper constant bits are NOT detected.

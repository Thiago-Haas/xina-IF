...
-- Must match traffic_gen_datapath_lfsr polynomial/taps so TG and TM stay in lock-step.
entity traffic_mon_datapath_lfsr is
  generic(
    p_WIDTH : positive := 32
  );
  port(
...
architecture rtl of traffic_mon_datapath_lfsr is
  function next_lfsr(x : std_logic_vector) return std_logic_vector is
    variable v  : std_logic_vector(x'range) := x;
    variable fb : std_logic;
  begin
    -- Same taps as traffic_gen_datapath_lfsr (example taps; adjust if you use a specific polynomial).

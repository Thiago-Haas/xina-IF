...
-- Convert hex Characters to utf-8 chars
entity utf8_hex is
  port
  (
    ctl_writelf_i : in  std_logic;
...
begin
  utf_data_o <= x"0A" when ctl_writelf_i = '1' else
                x"30" when data_i = x"0" else
                ...
                x"61" when data_i = x"a" else
                x"62" when data_i = x"b" else
                x"63" when data_i = x"c" else
                x"64" when data_i = x"d" else
                x"65" when data_i = x"e" else
                x"66"; --when data_i = x"f"; -- x"66" is ff         

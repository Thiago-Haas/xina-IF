...
-- * encode controller
-- * encode datapath
entity manager_uart_encode_core_block is
  generic (
...
architecture rtl of manager_uart_encode_core_block is
  ...
  signal dp_rx_count_w   : std_logic_vector(c_TM_COUNTER_WIDTH - 1 downto 0);
  signal dp_ok_count_w   : std_logic_vector(c_TM_COUNTER_WIDTH - 1 downto 0);
...
begin
  ...
  begin
    u_manager_uart_encode_critical_tmr: entity work.manager_uart_encode_critical_tmr
      generic map(
        G_REPORT_PERIOD_PACKETS => G_REPORT_PERIOD_PACKETS
      )
      port map(
  ...
  u_manager_uart_encode_ctrl: entity work.manager_uart_encode_ctrl
    generic map(
      G_REPORT_PERIOD_PACKETS => G_REPORT_PERIOD_PACKETS
    )
    port map(
      ...
      dp_rx_count_o   => dp_rx_count_w,
      dp_ok_count_o   => dp_ok_count_w,
  ...
  u_manager_uart_encode_datapath: entity work.manager_uart_encode_datapath
    port map(
      ACLK    => ACLK,
      ARESETn => ARESETn,
      ...
      rx_count_i   => dp_rx_count_w,
      ok_count_i   => dp_ok_count_w,
      flags_i      => dp_flags_w,
